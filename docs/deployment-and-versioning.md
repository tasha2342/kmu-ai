...
python3 scripts/loadtest.py --mode chatbot --concurrency 16   # 전 경로
```

**vLLM 직접 (모델 자체 한계)** — 256토큰 생성 기준
...
동시성 128까지 실패 0. 이 지점에서 두 GPU 모두 `utilization.gpu 100%` 라
**여기가 연산 한계**다. KV 캐시(9.3~304)가 아니라 GPU 연산이 먼저 찬다.
동시성이 8배(16→128) 늘 때 처리량은 4배 늘고 요청당 속도는 절반으로 떨어진다 —
배칭이 정상 동작한다는 뜻이다.

**챗봇 전 경로 (의도 분류 + 검색 + 응답 생성)** — 응답당 평균 350토큰
...
**실질 한계는 GPU 가 아니라 TTFT 다.** 모델만 보면 동시성 128에서도 첫 토큰이
0.3초에 나오지만, 챗봇을 거치면 동시성 32에서 3.4초다. 차이는 앱 계층에 있다.

구간을 재 보면 **검색(임베딩 포함)은 p50 168ms 로 TTFT 의 8% 에 불과하다.**
나머지 92% 는 `generate` 앞에 있는 비스트리밍 보조 LLM 호출 두 개
(`condense_query`, `classify_intent`)가 먹는다. gemma-4-31b-it 은 thinking 을 끌 수 없어
사고 토큰이 먼저 소비되는데, 그동안 사용자 화면에는 아무것도 뜨지 않는다.

정리하면:

- **동시 16명까지** — TTFT 2초 내외. 문제 없다.
- **동시 32명** — TTFT 3.4초(p90 4.9초). 쓸 수는 있지만 체감이 느려진다.
- **그 이상** — 모델은 더 받지만 앱 계층이 먼저 막힌다. 늘리려면 GPU 도
  임베딩도 아니고 **보조 LLM 호출**을 손봐야 한다.

> 측정 중 동시성 16에서 총 처리량이 80 tok/s 로 찍힌 적이 있는데, 재측정하니
> 450 tok/s 였다. 요청 한 건이 늦게 끝나면 `총 tok/s = 출력토큰합 ÷ 전체 시간`
> 이 통째로 끌려 내려간다. **한 번 재고 판단하지 말 것.**
