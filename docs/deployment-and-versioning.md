...
한 번의 질의에 LLM 호출이 두 번(의도 분류 → 응답 생성) 들어가고, 그 사이에
쿼리 임베딩이 컨테이너 안 CPU 에서 돈다(`app/utils/embedder.py`). 부하 중
API 컨테이너 CPU 가 순간 690% 까지 튀는 게 이 구간이다.

정리하면:

- **동시 16명까지** — TTFT 2초 내외. 문제 없다.
- **동시 32명** — TTFT 3.4초(p90 4.9초). 쓸 수는 있지만 체감이 느려진다.
- **그 이상** — 모델은 더 받지만 앱 계층이 먼저 막힌다. 늘리려면 GPU 가 아니라
  API 워커 수와 임베딩 경로를 손봐야 한다.
